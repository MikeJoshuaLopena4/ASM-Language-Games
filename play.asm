...
; Public Procedures{
	...
	proc ClearHalfScreen
		...
		ClearHalfScreen_2:
			mov dl, ' '
			mov ah, 2h   ; write CVharacter cursor position
			int 21h
			loop ClearHalfScreen_2
			pop cx
			mov dl, 0Ah    ; next line
			mov ah, 2h
			int 21h
			loop ClearHalfScreen_1
			ret
		endp ClearHalfScreen
;}
...
; Private Chonker Procedures{
	...
	proc playChonkerGame	
		...
		moveleft:
			...
			mov AL, ' '      ; draw the tunnel as an "X"
			mov BH, 0        ; page 0
			mov BL, 0Ah     ; 7=white
